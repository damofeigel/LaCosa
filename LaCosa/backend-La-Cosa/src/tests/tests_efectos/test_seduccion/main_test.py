...
lista_jugadores[0].agregar_carta_mano(CardSchema(id=60, 
                                                 num_players=4, 
                                                 type="Accion", 
                                                 name="Seduccion", 
                                                 effect="Intercambia una carta con cualquier jugador de tu eleccion que no este"\
                                                    " en Cuarentena. Tu turno termina", 
                                                 target="any"))
...
async def intercambiar_carta(id_partida: int, id_jugador: int, trade_data_ini: IntercambioIn):
    ...
    _jugador_ = _partida_.buscar_jugador(id_jugador)
    
    # Obtengo el jugador con quien se quiere intercambiar
    id_objetivo = obtener_objetivo_intercambio(_partida_, _jugador_)
    _jugador_objetivo_ = _partida_.buscar_jugador(id_objetivo)
    _jugador_.set_id_objetivo_intercambio(-1)

    if not verificar_intercambio(_jugador_, _jugador_objetivo_.get_rol(), trade_data_ini.id_carta):
    ...
    _jugador_.set_id_carta_intercambio(trade_data_ini.id_carta)
    ...
    evento_mano_bloqueda : EventoMano = armador_evento_mano(jugador=_jugador_,
                                                            instancia="Esperar", 
                                                            context="Esperando")
    _partida_.post_event(id_jugador=id_jugador, 
    ...
    _partida_.post_event(id_jugador=id_objetivo, 
                         event=evento_pedido_intercambio, 
                         event_name="Pedido_intercambio")

    # evento_mano_intercambiables : EventoMano = armador_evento_mano(jugador=_jugador_objetivo_,
    #                                                                instancia="Intercambiar",
    #                                                                context="Intercambiables")
    evento_mano_intercambiables : EventoMano = armador_evento_mano(jugador=_jugador_objetivo_,
    ...
    # - Retornar sólo el código de estado
    return
...
async def resolver_intercambio(id_partida: int, id_jugador: int, trade_data_res: RespuestaIntercambioIn):
    ...
    index = lista_partidas.get_index_by_id(id_param=id_partida)
    _partida_ = lista_partidas.get_partida_by_index(index)
    jugador_resolvente = _partida_.buscar_jugador(id_jugador)
    # - Buscar la carta que le quieren dar a este jugador
    #   > El que inicia el intercambio es siempre el jugador del turno
    jugador_ofrecedor: Jugador = _partida_.buscar_jugador_por_posicion(_partida_.get_turno())
    ...
    # - Armar y enviar eventos de las nuevas manos, bloqueadas porque ya no pueden hacer nada
    ...
    _partida_.post_event(id_jugador=jugador_ofrecedor.get_id(), 
                         event=evento_mano_bloqueda1, 
                         event_name="Mano")

    _partida_.post_event(id_jugador=jugador_resolvente.get_id(), 
    ...
    armar_broadcast(_partida_=_partida_, msg=evento_intercambio_resuelto, event_name="Respuesta_intercambio")
